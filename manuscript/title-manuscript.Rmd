...
This resulted in `r length(unique(titles$Journals))`
...
```{r HDIs, include = FALSE, warning = FALSE}
HDIs <- titles %>% dplyr::select(GPE, HDI) %>%
...
with_nations$interaction_term <- with_nations$prop * with_nations$SJR_rank
# kruskal-wallis test
...
random chance
(Kruskall-Wallis rank sum test, *p* = `r round(cit2rank$p.value, 3)`).
