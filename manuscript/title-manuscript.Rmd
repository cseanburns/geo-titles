...
# Import data
titles <- read.csv("~/workspace/geo-titles/data/titles.csv", header = TRUE, sep = ",")
```

```{r divide-data, include = FALSE, warning = FALSE}
# data prep
# subset of data that does not include titles with place names
without_nations <- dplyr::filter(titles, titles_has_country == 0)
without_nations <- dplyr::select(without_nations, Citations, Journals, SJR)
# subset of data that does includes titles with place names
with_nations <- dplyr::filter(titles, titles_has_country == 1 & HDI != "NA")
with_nations <- dplyr::select(with_nations, Citations, Journals, GPE,
                              HDI, HDI_Level, CountryRegionMulti,
                              Averaged, pos, prop)
```
...
where retrieval technology based on relevance algorithms
begins to dominate searching
index keywords in various bibliographic fields.
...
n_nations <- titles %>% dplyr::select(GPE) %>%
  dplyr::filter(!is.na(GPE)) %>% summarize(count = n())
...
ranked in the first quartile (Q1) of the LIS category, but
removed two chemistry heavy journals
(*Journal of Chemical Information and Modeling* and
*Journal of Cheminformatics*),
resulting in `r length(unique(titles$Journals))`
...
We retrieved `r n_total` article records
across the `r length(unique(titles$Journals))` journals.
...
While the bibliographic records are primarily in English,
some titles contained translations from other languages:
Bosnian, Croatian, Portuguese, and Spanish.
To reduce the duplication of titles,
we kept the English translations and
removed the titles that were written
in the original language.
We kept some titles written in Spanish
since there were no translations for these titles.
...
no_HDIs <- titles %>% dplyr::select(GPE, HDI) %>%
  dplyr::filter(!is.na(GPE)) %>%
...
averaged_HDIs <- titles %>% dplyr::select(Averaged) %>%
  dplyr::filter(grepl('Y', Averaged)) %>%
  dplyr::summarize(count = n())

mean_HDI <- titles %>% dplyr::select(HDI) %>%
  dplyr::summarize(mean = mean(HDI, na.rm=TRUE)) %>%
  round(3)

median_HDI <- titles %>% dplyr::select(HDI) %>%
  dplyr::summarize(median = median(HDI, na.rm=TRUE)) %>%
  round(3)
...
for each of the
`r length(unique(titles$Journals))`
...
We used spaCy 
We manually reviewed all titles and 
identified 54 titles where spaCy labeled a titled
as containing a place name.
...
#tiff("title-manuscript_files/figure-html/fig2.tif",
#    width = 3840, height = 2160, pointsize = 12, res = 300)
...
#tiff("title-manuscript_files/figure-html/fig3.tif",
#    width = 3840, height = 2160, pointsize = 12, res = 300)
...
#tiff("title-manuscript_files/figure-html/fig4.tif",
#    width = 3840, height = 2160, pointsize = 12, res = 300)
...
#tiff("title-manuscript_files/figure-html/fig5.tif",
#    width = 3840, height = 2160, pointsize = 12, res = 300)
