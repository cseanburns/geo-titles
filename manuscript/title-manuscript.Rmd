...
with_nations <- dplyr::select(with_nations, id, doc_id, HDI, HDI_Level,
                              CountryRegionMulti, Averaged, GPE,
                              Citations, False_Positive, False_Negative,
                              Titles, Journals, SJR, pos, prop)
```
...
n_total <- length(titles$id)
n_nations <- length(titles$id[titles$titles_has_country])
...
HDIs <- table(titles$HDI >= 0)

nations_no_HDIs <- n_nations - HDIs
...
Our analysis thus includes `r n_total` article records
...
```{r nations-table, echo = FALSE}
nation_freq_table <- with_nations %>%
  count(GPE, sort = TRUE) %>%
  arrange(desc(n)) %>%
  head(20)
knitr::kable(nation_freq_table,
    caption = "Table 1. Twenty Most Frequent Geographical Political Areas Referred to in Article Titles for LIS Journals, 2018-2020") %>%
    # kable_styling() %>%
    footnote(general = "Spain was mentioned in 119 article titles in the journal Profesional de la Información. There was a total of 177 geographical areas or combinations of areas identified in the titles.")
```
...
The HDI includes regional values (*n* = `r regions`):
...
medium (\>= 0.55),
and low (\<= 0.549).
There were `r HDIs` records with HDIs.
...
(*m* = `r sjr_unique_mean`; *mdn* = `r sjr_unique_median`),
...
(*n* = `r length(titles$Citations[titles$titles_has_country == 1])`)
are cited less frequently
than articles without geographical names
(*n* = `r length(titles$Citations[titles$titles_has_country == 0])`).
...
```{r sub-hypo-1, warnings=FALSE, include=FALSE}
cit2places <- cor.test(with_nations$Citations, with_nations$prop, method = "spearman", exact=FALSE)
```
...
(Spearman *rho* = `r round(cit2places$estimate, 3)`),
...
ggplot(with_nations, aes(x = prop, y = Citations)) +
...
(Spearman *rho* = `r round(cit2HDI$estimate, 3)`;
*p* = `r round(cit2HDI$p.value, 3)`).

```{r sub-hypo-3, include=FALSE}
fit.1 <- lm(titles$Citations ~ titles$titles_has_country * titles$SJR)
```
...
(*B* = `r round(fit.1$coefficients[2], 3)`, *p* = 0.002),
...
`r round(fit.1$coefficients[3], 3)` (*p* < 0.001)
...
increased only by *B* = `r fit.1$coefficients[4]`
(*p* = 0.027).

```{r echo=FALSE}
fit.1 %>% tidy() %>% mutate(
    p.value = scales::pvalue(p.value),
    term = c("Intercept", "Title Has Country", "SJR", "Interaction")) %>%
        kable(
            caption = "Table 2. Coefficient Estimates for Model Estimating the Interaction between the Presence of a Place Name in a Title and a Journal's SJR",
...
journal_summary <- titles %>%
  ...
  summarise(
    total_articles = n(),
    articles_with_places = sum(!is.na(GPE)),
...
(Spearman *rho* = `r round(places2SJR$estimate, 3)`;
*p* = `r round(places2SJR$p.value, 3)`) (Fig. 3).

```{r fig-3, fig.cap = "Fig. 3: There is a statistically weak relationship between the frequency of articles with place names in titles in journals and the journals' impact score.", echo = FALSE, message = FALSE}
...
nation_summary <- with_nations %>%
  ...
  group_by(GPE) %>%
  summarise(
    avg_position = median(prop, na.rm = TRUE),
    HDI = first(HDI),
    total_articles = n()
  ) %>%
...
(Spearman *rho* = `r round(totart2pos$estimate, 3)`;
...
```{r fig-4, fig.cap = "Fig. 4: The relationship between average location of nations named in article title and HDI. Only nations appearing in at least ten articles are displayed (n = 21).", echo = FALSE}
tiff("title-manuscript_files/figure-html/fig4.tif",
    width = 3840, height = 2160, pointsize = 12, res = 300)
ggplot(nation_summary, aes(x=avg_position, y=HDI)) +
  geom_point(aes(color=GPE), alpha=0.7) +
  geom_text(aes(label=GPE), vjust=-1, hjust=1, size=3, check_overlap = TRUE) + 
...
journal_hdi_summary <- with_nations %>%
  ...
  summarise(
    HDI = first(HDI),
    total_articles_with_places = sum(!is.na(prop))
  )
...
(Spearman *rho* = `r round(HDI2journals$estimate, 3)`;
