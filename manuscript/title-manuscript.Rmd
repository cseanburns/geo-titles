...
(*rho* = `r cit2places$estimate`; *p* = `r cit2places$p.value`)
indicating that the closer a place name was to the end
of a title,
the more citations it received.

```{r fig-1, fig.cap = "Fig. 1: Relationship between Citations and Position of Place Name in Title", echo = FALSE}
...
nor strong (*rho* = `r cit2HDI$estimate`; *p* = `r cit2HDI$p.value`).
...
random chance (*p* = `r cit2rank$p.value`).
...
(*rho* = `r places2SJR$estimate`; *p* `r places2SJR$p.value`).
...
```{r fig-2, fig.cap = "Fig. 2: Relationship between Journal Impact and Proportion of Articles with Place Names", echo = FALSE, message = FALSE}
...
(*rho* = `r totart2pos$estimate`; *p* = `r totart2pos$p.value`).
...
```{r fig-3, fig.cap = "Fig. 3: Relationship between Average Location of Nation Name in Article Title and HDI", echo = FALSE}
ggplot(nation_summary, aes(x=HDI, y=avg_position)) +
  ...
  labs(title="Median Position vs. HDI",
       x="HDI",
       y="Median Position of Nation in Title") +
  theme_minimal() +
...
(*rho* = `r HDI2journals$estimate`; *p* = `r HDI2journals$p.value`)
...
```{r fig-4, fig.cap = "Fig. 4: Relationship Between SJR and HDI of Nations Mentioned in Article Titles", echo = FALSE, message = FALSE, warning = FALSE}
ggplot(journal_hdi_summary, aes(x = HDI,
                                y = SJR)) +
  ...
  xlab("HDI of Mentioned Nations") +
  ylab("Journal's SJR (Impact)") +
  theme_minimal() +
